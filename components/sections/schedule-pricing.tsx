...
function ScheduleCard({ title, subtitle, schedule }: { title: string; subtitle: string; schedule: ScheduleDay[] }) {
  const compactSchedule = schedule
    .filter((day) => day.day !== "DOMENICA")
    .map((day) => ({ ...day, classes: day.classes.slice(0, 4) }))
    ...
    <div className="rounded-sm border border-border/80 bg-card p-5 shadow-sm sm:p-7">
      ...
      <div className="md:hidden">
        <div className="grid grid-cols-3 gap-2 pb-4">
          {compactSchedule.map((day, index) => (
            <button
              key={day.day}
              onClick={() => setActiveDayIndex(index)}
              className={`border py-2 text-[10px] font-bold tracking-tight uppercase transition-colors ${activeDayIndex === index ? "border-primary bg-primary text-primary-foreground" : "border-border bg-transparent text-muted-foreground hover:border-primary/40"}`}
            >
        ...
        <div className="mt-4 space-y-3">
          {compactSchedule[activeDayIndex].classes.map((classItem) => (
...
export function ScheduleSection({ isVisible, setSectionRef }: AnimatedSectionProps) {
  return (
    <section id="orari" ref={(element) => setSectionRef("orari", element)} style={{ background: "radial-gradient(ellipse at 15% 50%, #111111 0%, #080808 60%, #0F0F0F 100%)" }} className={`py-20 transition-[transform,opacity] duration-500 sm:py-28 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
...
export function PricingSection({ isVisible, setSectionRef }: AnimatedSectionProps) {
  return (
    <section id="prezzi" ref={(element) => { setSectionRef("prezzi", element) }} style={{ background: "radial-gradient(ellipse at 85% 40%, #161616 0%, #111111 60%, #080808 100%)" }} className={`bg-card py-24 transition-[transform,opacity] duration-500 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-16 px-4 text-left lg:px-0 lg:text-center">
